...
from .models import db, User, Short_URL, Redirect_Types
...
LOCKED_SLUG = ['login', 'register', 'panel', 'logout', 'generate-qr-code', 'analytics']
...
@endpoints.route('/generate-qr-code/')
@login_required
def generate_qr_code_endpoint():
    # Generate QR code based on the current user's URL
    user_id = current_user.id
    short_url = Short_URL.query.filter_by(user_id=user_id).first()
    url =short_url.org_url
    ...
    return render_template('qr_code.html', qr_image=img_str)


@endpoints.route('/analytics/')
@login_required
def analytics_endpoint():
    # Retrieve analytics data for the current user from the database
    click_data = db.session.query(Short_URL.slug_text, Short_URL.visits, 
                                  Short_URL.org_url, Short_URL.visits_timestamp)\
                           .join(Short_URL.user)\
                           .filter(User.id == current_user.id)\
                           .all()

    # Display the analytics data
    return render_template('analytics.html', click_data=click_data)